  ...
  bump(version) {
    ...
    const targets = [
      ...
      },
      {
        ...
        infoPlistPath: './macos/Chronos Companion/Info.plist',
      },
      {
        name: 'ChronosWidget4WeeksOverview',
        infoPlistPath: './macos/ChronosWidget4WeeksOverview/Info.plist',
      },
    ];

    targets.forEach(target => {
      // Get CFBundleVersion from the Info.plist file
      let infoPlistFile = fs.readFileSync(target.infoPlistPath, 'utf8');
      ...
      fs.writeFileSync(target.infoPlistPath, infoPlistFile);
    });
